...
def create_tf_example(group, path, label_map):
    ...
    for index, row in group.object.iterrows(): #uma anotação em um filename pode haver mais de um objeto, então percorre-se uma lista. No meu caso, a maioria tem apenas um objeto como anotação
        xmins.append(row['xmin']) #insere a posição relativa de xmin.
        xmaxs.append(row['xmax']) #insere a posição relativa de xmax
        ymins.append(row['ymin']) #insere posição relativa de ymin
        ymaxs.append(row['ymax']) #insere posição relativa de ymax
